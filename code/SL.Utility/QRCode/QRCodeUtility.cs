...
namespace SL.Utility
{
    public class QRCodeUtility
    {
        ...
        /// <returns></returns>
        public static Image CreateQRCode(string Content, QRCodeEncoder.ENCODE_MODE QRCodeEncodeMode, QRCodeEncoder.ERROR_CORRECTION QRCodeErrorCorrect, int QRCodeVersion, int QRCodeScale, int size, int border)
        {
            try
            {
                ...
                if (size > 0)
                {
                    ...
                    if (image.Width <= size)
                    {
                        ...
                        {
                            //新建空白绘图
                            Bitmap panel = new System.Drawing.Bitmap(size, size);
                            Graphics graphic0 = System.Drawing.Graphics.FromImage(panel);
                            int p_left = 0;
                            int p_top = 0;
        ...
        /// <param name="border">图片白边（像素），当size大于0时有效</param>
        /// <returns></returns>
        public static Image CreateQRCode(string Content, int size = 0, int border = 1)
        {
            return CreateQRCode(Content, QRCodeEncoder.ENCODE_MODE.BYTE, QRCodeEncoder.ERROR_CORRECTION.M, 0, 5, size, border);
        }
